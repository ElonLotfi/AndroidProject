...
public class TestCircle {
    ...
    @Rule
    public IntentsTestRule<DrawingCircle> intentsTestRule = new IntentsTestRule<>(DrawingCircle.class);
    ...
    @Test
    public void perfectCircle() throws Exception {

        doAnswer(new Answer() {
            @Override
            public Object answer(InvocationOnMock invocationOnMock) throws Throwable {

                AttributionScoreCircle scoreCircle = new AttributionScoreCircle(drawingCanvas.getShapePoint().getPoints());
        ...
        onView(withId(R.id.drawingCanvas5)).perform(singleTouchAt(-1, -3, MotionEvent.ACTION_MOVE));
        onView(withId(R.id.drawingCanvas5)).perform(singleTouchAt(1,-3, MotionEvent.ACTION_MOVE));
        onView(withId(R.id.drawingCanvas5)).perform(singleTouchAt(-3,-1, MotionEvent.ACTION_MOVE));
        onView(withId(R.id.drawingCanvas5)).perform(singleTouchAt(3,-1, MotionEvent.ACTION_MOVE));
        onView(withId(R.id.drawingCanvas5)).perform(singleTouchAt(-3,1, MotionEvent.ACTION_MOVE));
        onView(withId(R.id.drawingCanvas5)).perform(singleTouchAt(1,3, MotionEvent.ACTION_MOVE));
        onView(withId(R.id.drawingCanvas5)).perform(singleTouchAt(3,1, MotionEvent.ACTION_MOVE));
        onView(withId(R.id.drawingCanvas5)).perform(singleTouchAt(-1,3, MotionEvent.ACTION_MOVE));
        onView(withId(R.id.buttonCircle)).perform(click());
        connexion.emitShape(drawingCanvas.getShapePoint().convertToJson());

        GlobalState gs = (GlobalState) intentsTestRule.getActivity().getApplicationContext();
        assertEquals(true,gs.getClient().getScore()==3);
